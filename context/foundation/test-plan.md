...
|---|---|---|---|---|---|---|
| 1 | Database guarantees in CI | Prove no double booking, no silent booking loss on schedule edits and no cross-tenant access — automatically, on every push, not by hand | #1, #2, #4 | integration (database) | change opened | context/changes/testing-database-guarantees-in-ci/ |
...
| integration (database) | local Supabase (Postgres) via Supabase CLI + psql scripts | CLI 2.98 | exists as manual scripts; automated in CI by §3 Phase 1 |
...
| integration (database guarantees) | CI (local Supabase on runner) | required after §3 Phase 1 | double booking, silent booking loss, cross-tenant access |
...
- TBD — see §3 Phase 1 for the double-booking / schedule-guard / RLS
  isolation pattern (how to run against the local stack and in CI).
